model showBaseMotion
  ...
  Modelica.Blocks.Sources.Sine RoadMeasureGyro(amplitude = 0.017, f = 2) annotation(
  ...
  seondorderBaseMotion mode2Az(I = 0.8, c = 20, k = 650000)  annotation(
...
equation
  ...
  connect(sum1.y, toMicroRad1.u) annotation(
    Line(points = {{47, -66}, {70, -66}}, color = {0, 0, 127}));
  connect(seondorderBaseMotion1.y, toMicroRad.u) annotation(
    Line(points = {{6, 2}, {38, 2}}, color = {0, 0, 127}));
  connect(sum1.y, toMicroRad1.u) annotation(
    Line(points = {{50, -66}, {70, -66}}, color = {0, 0, 127}));
  connect(RoadMeasureGyro.y, integrator.u) annotation(
    Line(points = {{-84, -54}, {-78, -54}}, color = {0, 0, 127}));
  connect(integrator.y, mode1Az.u) annotation(
    Line(points = {{-54, -54}, {-22, -54}}, color = {0, 0, 127}));
  connect(integrator.y, mode2Az.u) annotation(
    Line(points = {{-54, -54}, {-38, -54}, {-38, -84}, {-22, -84}}, color = {0, 0, 127}));
  connect(mode1Az.y, sum1.u[1]) annotation(
    Line(points = {{-2, -54}, {8, -54}, {8, -66}, {24, -66}}, color = {0, 0, 127}));
  connect(mode2Az.y, sum1.u[2]) annotation(
    Line(points = {{-2, -84}, {8, -84}, {8, -66}, {24, -66}}, color = {0, 0, 127}));
  annotation(
...
See how resonance occurs at 105.2Hz."), Text(origin = {0, -28}, extent = {{-104, 12}, {104, -12}}, textString = "Superposition of the modes of an axis for example azimuth"), Text(origin = {-70, -98}, extent = {{-38, 6}, {38, -6}}, textString = "Integrates the measured gyro deviations 
of the road input to give an angle disturbance"), Line(origin = {-80, -79}, points = {{-4, -13}, {4, 13}}, arrow = {Arrow.None, Arrow.Filled})}, coordinateSystem(extent = {{-120, 80}, {120, -100}})),
  version = "");
end showBaseMotion;
